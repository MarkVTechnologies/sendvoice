...
| PDF rendering | Server-side headless browser render (`puppeteer-core` against system Chrome, dev-only) → Postgres `bytea` for now | §9.2 P0: client-side PDF generation is explicitly rejected. The Chrome-reuse and Postgres-storage choices are both deliberate dev-machine shortcuts, not production decisions — see `src/services/pdf.ts`. Real object storage becomes necessary once the hosted invoice page needs a public URL |
...
- [ ] Remaining PDF templates: 2–3 more distinct, professionally typeset designs (PRD §8.5 — one exists today); real object storage for PDFs instead of Postgres `bytea`; production-grade rendering (warm browser instance/pool instead of a ~10s cold Chrome launch per request)
...
Also this session: real TTFI instrumentation (server-computed from `Tenant.createdAt`/`Document.approvedAt`, gated to a tenant's first invoice only), timed against a real browser-driven signup-to-first-send run — **23.3s**, well under the phase's own 90s exit criterion.
